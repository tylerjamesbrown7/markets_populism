...
oes <- st_read(
  "/Users/tylerbrown/R/Thesis Project/data/AUT_adm3.shp")


oes <- oes %>% rename(bundesland = NAME_1, district = NAME_2, comm_name = NAME_3, comm_id = CCN_3)
...
dVS_df <- incl_df %>% select(comm_id, nuts3, fpo, dVS, Tx2, year,ruemp, dist_exposure, dist_id, in_tot,pop_tot,elect_year, years_to_elect) %>% as.data.frame()

oes
map_merge <- merge(oes, aust, by = 'comm_id', all.x=TRUE)
map_merge %>% names

unemp_df_map <- dVS_df %>% filter(!is.na(dVS)) %>% group_by(dist_id) %>% summarise(ruemp = mean(ruemp, na.rm=TRUE))
unemp_df_map <- merge(dist_map, unemp_df_map)
...
nuts_map %>%  ggplot()+
  geom_sf(data = map_merge, aes(fill = freight_unload), linewidth = 0)+
  facet_wrap(~year)
  


nuts_map %>%  ggplot()+
  geom_sf(data = map_merge %>% filter(!is.na(dVS)), aes(fill = freight_unload), linewidth = 0)


aust %>% ggplot(aes(x = year, y = freight_unload, color = nuts3))+
